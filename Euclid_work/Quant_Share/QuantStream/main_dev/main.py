...
st.title(":blue[Quant Share] Factor Kit :red[Dev]")
st.divider()
...
with factor_backtest:
    ...
    factor_backtest_date_col = text_input.text_input('因子日期列名称', value='tradeDate', key='backtest_factor_date')
    factor_backtest_uploaded = file_upload.file_uploader("## 上传因子数据（宽表格式）", type=['h5', 'csv'], key='backtest_factor_uploaded')

    detect = text_input.checkbox("自动检测日期列", key='backtest_date_detect')
    if detect and factor_backtest_uploaded is not None:
        factor, factor_backtest_date_col = read_file(factor_backtest_uploaded, auto=True)
        st.info(f"自动检测的日期列名为：{factor_backtest_date_col}")
    elif factor_backtest_uploaded is not None:
        factor, factor_backtest_date_col = read_file(factor_backtest_uploaded, factor_backtest_date_col)
    ...
    if col1.button("分组回测指标计算"):
        with st.spinner('Wait for it...'):
            outMetrics, group_res = bkTest(score, start_date, end_date, bench_code, key='factor')
...
with factor_analysis:
    ...
    with date_name_col:
        factor_analysis_factor_date_col = st.text_input('因子日期列名称', value='tradeDate', key='factor_analysis_factor_date')
        factor_analysis_factor = st.file_uploader("## 上传因子数据（宽表格式）", type=['h5', 'csv'], key='factor_analysis_factor_uploaded')

    with file_upload_col:
        factor_analysis_rtn_date_col = st.text_input('回报率日期列名称', value='tradeDate', key='factor_analysis_rtn_date')
        factor_analysis_rtn = st.file_uploader("## 上传回报率数据（宽表格式）", type=['h5', 'csv'], key='factor_analysis_rtn_uploaded')

    detect = st.checkbox("自动检测日期列", key='factor_analysis_date_detect')

    if detect and factor_analysis_factor is not None:
        factor, factor_analysis_factor_date_col = read_file(factor_analysis_factor, auto=True)
        if factor is not None:
            factor, score = factor2score(factor)
            st.info(f"因子数据自动检测的日期列名为：{factor_analysis_factor_date_col}")
        if factor_analysis_rtn is not None:
            rtn, factor_analysis_rtn_date_col = read_file(factor_analysis_rtn, auto=True)
            st.info(f"回报率数据自动检测的日期列名为：{factor_analysis_rtn_date_col}")
    elif factor_analysis_factor is not None:
        factor, factor_analysis_factor_date_col = read_file(factor_analysis_factor, factor_analysis_factor_date_col)
        if factor is not None:
            factor, score = factor2score(factor)
        if factor_analysis_rtn is not None:
            rtn, factor_analysis_rtn_date_col = read_file(factor_analysis_rtn, factor_analysis_rtn_date_col)
...
with factor_barra:
    ...
    if st.button("BARRA因子回测"):
        with st.spinner('Wait for it...'):
            factor = get_factor(barra, factor_name)
            factor, score = factor2score(factor, verbose=False)
            outMetrics, group_res = bkTest(score, start_date, end_date, bench_code, key=factor_name)
            st.write("### 回测结束, 各组指标如下")
            st.dataframe(outMetrics.set_index('group'))
            st.success('分组回测指标计算完成!', icon="✅")
            streamlit_echarts.st_pyecharts(nav_plot(group_res, plot_begin, plot_end, bench_code), height="500px",
