...
class FactorData:
    ...
    def net_cf_inv(self):
        ...
        df = get_data(
            tableName="fundamentals_cashflow",
    ...
    def PETTM(self):
        df = get_data(
            tableName="trading_derivative_indicator",
            begin=self.beginDate,
            end=self.endDate,
    ...
    def NPCUT(self):
        ...
        """
        NPCUT = get_data(
            tableName="deriv_finance_indicator",
            begin=get_tradeDate(self.beginDate, -2 * 365),
            end=self.endDate,
...
class FactorBase(FactorData):
    ...
    def adjust(self, df: pd.DataFrame, method: str = None):
        res = pd.DataFrame(
            data=0,
            columns=df.columns,
            index=pd.date_range(self.beginDate, self.endDate),
        )
    ...
    def align_data(data_lst=None, clean: bool = True, *args):
        ...
        if len(data_lst) > 2:
            mut_date_range = sorted(
                reduce(lambda x, y: x.intersection(y), (df.index for df in data_lst))
            )
            if dims == 2:
                mut_codes = sorted(
                    reduce(
                        lambda x, y: x.intersection(y), (df.columns for df in data_lst)
                    )
                )
            ...
            if dims == 2:
                mut_codes = sorted(
                    data_lst[0].columns.intersection(data_lst[1].columns)
                )
                data_lst = [df.loc[mut_date_range, mut_codes] for df in data_lst]
            elif dims == 1:
                data_lst = [df.loc[mut_date_range, :] for df in data_lst]
    ...
    def winsorize(
        ...
        if mode == "mad":
            ...
            return winsorize(
                np.arange(8), limits=kwargs.get("limits", [0.1, 0.1]), nan_policy="omit"
            )
    ...
    def capm_regress(self, X: pd.DataFrame, Y: pd.DataFrame, window=504, half_life=252):
        ...
        beta, alpha, sigma = self.rolling_regress(
            Y, X, window=window, half_life=half_life
        )
        return beta, alpha, sigma

    def rolling(self, datdf, window, half_life=None, func_name="nansum", weights=None):
        ...
        try:
            res = self.pandas_parallelcal(datdf, self.nanfunc, args=args, window=window)
        except Exception:
            print("Calculating under single core mode...")
            res = self.rolling_apply(
                datdf, self.nanfunc, args=args, axis=0, window=window
            )
        return res
