...
exit_info = pd.read_sql(
    "select symbol, max(Date(record_time)) as date from fundamentals_income group by symbol;",
    con=postgres_engine(),
)
    ...
    for symbol in t:
        ...
        try:
            ...
            if len(data) > 0:
                ...
                postgres_write_data_frame(data, "fundamentals_income", update=True, unique_index=["symbol", "pub_date", "rpt_date"], record_time=True)
        except GmError:
