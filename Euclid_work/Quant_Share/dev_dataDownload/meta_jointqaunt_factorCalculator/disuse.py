...
def GetOringinalData(beginDate: str = None, endDate: str = None, save: bool = True, read_only: bool = False, **kwargs):
    ...
    res = ['financial_data', 'market_sheet', 'market_financial_sheet', 'ResConSecCorederi_sheet']
    ...
    if read_only:
        saved_data = {}
        for k in res:
            if k == 'market_financial_sheet': continue
            saved_data[k] = get_data(k, begin=beginDate, end=endDate)
        return saved_data

    filenames = ['fundamentals_balance',
                 'balance_sheet',
                 'fundamentals_income',
    ...
    with tqdm(filenames, leave=True) as t:
        for file in t:
            ...
            if file not in ['share_change', 'ResConSecCorederi']:
                if file == 'ResConSecCorederi': continue
    ...
    for k in res:
        ...
        if k != 'market_financial_sheet':
            # continue
            save_data_Y(tmp_df, date_column_name='rpt_date', tableName=k,
                        _dataBase_root_path=dataBase_root_path_JointQuant_prepare, reWrite=True)
        else:
            save_data_h5(tmp_df, name='{}'.format(k),
                         subPath="{}/{}".format(dataBase_root_path_JointQuant_prepare, k), reWrite=True)
