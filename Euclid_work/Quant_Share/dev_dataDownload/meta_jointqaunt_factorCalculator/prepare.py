...
class DataPrepare:
    def __init__(self, beginDate: str = None, endDate: str = None, **kwargs) -> None:
        ...
        self.beginDate = (
            beginDate if beginDate is None else get_tradeDate(date.today(), -365 * 4)
        )
    ...
    def income_cashflow_process(self, df):
        ...
        df = df.sort_values("rpt_date").drop_duplicates(
            subset=["symbol", "rpt_date"], keep="last"
        )
    ...
    def balance_sheet(self):
        ...
        fields.append("end_date")
        df = get_data(
            "balance_sheet", begin=self.beginDate, end=self.endDate, fields=fields
        )
        df = df.reset_index(drop=True)
        df[df == 0] = np.nan
        df = df.rename(columns={"end_date": "rpt_date"})
    ...
    def share_number_sheet(self):
        ...
        share_number_sheet["rpt_date"] = share_number_sheet[
            "rpt_date"
        ].dt.to_timestamp()
    ...
    def market_sheet(self):
        ...
        trading_derivative_indicator = trading_derivative_indicator.rename(
            columns={"end_date": "rpt_date"}
        )
        trading_derivative_indicator["rpt_date"] = trading_derivative_indicator[
            "rpt_date"
        ].dt.strftime("%Y-%m-%d")
        trading_derivative_indicator["rpt_date"] = pd.to_datetime(
            trading_derivative_indicator["rpt_date"]
        )
        market_sheet = pd.merge(
            market_sheet,
            trading_derivative_indicator,
    ...
    def market_financial_sheet(self):
        ...
        financial_sheet = get_data(
            "financial_sheet", begin=self.beginDate, end=self.endDate
        )

        market_sheet = get_data("market_sheet", begin=self.beginDate, end=self.endDate)

        fields = (
            useful_field(self.joint_quant_factor, "income_sheet")[3:]
        ...
        market_financial_sheet = pd.merge(
            market_sheet, financial_sheet, on=["symbol", "pub_date"], how="left"
        )
        ...
        market_financial_sheet.drop(
            ["share_circ_x", "share_circ_y"], axis=1, inplace=True
        )
