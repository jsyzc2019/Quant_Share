...
class DataPrepare:
    ...
    def get_limit_data(self):
        ...
        for coli in ['limitUpPrice', 'limitDownPrice']:
            self.TICKER[coli] = reindex(MktLimit_df.pivot(index='tradeDate', columns='ticker', values=coli))
...
class simpleBT:
    ...
    def backTest(self, Score, fee_rate=0.0008, group=1, dealPrice='close', plot=False):
        ...
        for date in tqdm(self.tickerData['Rtn'].index.to_list()):
            ...
            if date in Score.index.to_list():
                ...
                temp_pos = temp_pos * (1 + self.tickerData['Rtn'].loc[date])  # 直接使用close/close_pre - 1收益率进行更新
                temp_pos = temp_pos / temp_pos.sum()
                temp_pos.fillna(0, inplace=True)

            pos_out.loc[date] = temp_pos  # 记录个股仓位
            daily_rtn.loc[date] = pos_rtn + trade_rtn - fee.loc[date]  # 每日收益=持仓收益+交易后的收益-手续费
    ...
    def curveanalysis(cls, nav):
        ...
        number_of_years = (pd.to_datetime(nav.index[-1]).toordinal() - pd.to_datetime(nav.index[0]).toordinal()) / 356
